...
[Collection("api")]
public class AssetsControllerTests(OptionsProvider optionsProvider) : ControllerTests(optionsProvider)
{
    ...
    private async Task SeedWithTestBankAccount(string accountName = "Test Bank Account")
    {
        ...
        for (DateTime i = DateTime.UtcNow.AddMonths(-24).Date; i <= DateTime.UtcNow; i = i.AddDays(1))
            _testDatabase!.Context.BankEntries.Add(new BankAccountEntry(test.AccountId, 0, _nowUtc.AddDays(-1), _value += _valueChange, _valueChange));
    ...
    public async Task GetEndAssetsPerAccount_ReturnsList()
    {
        ...
        var result = await new AssetsHttpContext(Client).GetEndAssetsPerAccount(1, currency, _nowUtc);
    ...
    public async Task GetEndAssetsPerType_ReturnsList()
    {
        ...
        Authorize("TestUser", 1, UserRole.User);
        var currency = new Currency(1, "USD", "$");
        var result = await new AssetsHttpContext(Client).GetEndAssetsPerType(1, currency, _nowUtc);
        Assert.NotNull(result);
        Assert.Single(result);
    ...
    public async Task GetAssetsTimeSeries_ReturnsList()
    {
        ...
        Authorize("TestUser", 1, UserRole.User);
        var currency = new Currency(1, "USD", "$");
        var result = await new AssetsHttpContext(Client).GetAssetsTimeSeries(1, currency, _nowUtc.AddDays(-2), DateTime.UtcNow);
        Assert.NotNull(result);
