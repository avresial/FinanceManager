...
public partial class StockEntryConflictResolver
{
    ...
    private List<StockImportConflict> _selectedConflicts = [];
    private Dictionary<DateTime, List<StockImportConflict>> _conflictsByDay = [];
    ...
    protected override void OnInitialized()
    {
        ...
        {
            Logger.LogError(ex, "Error initializing {StockEntryConflictResolver} for account {AccountId}", nameof(StockEntryConflictResolver), AccountId);
        }
        _isLoading = false;
    }
    ...
    private async Task OnPickImported()
    {
        ...
        try
        {
            var resolvedImports = _selectedConflicts.Select(c => new ResolvedStockImportConflict(c.AccountId, true, c.ImportEntry, false, c.ExistingEntry?.EntryId))
                                        .ToList();
