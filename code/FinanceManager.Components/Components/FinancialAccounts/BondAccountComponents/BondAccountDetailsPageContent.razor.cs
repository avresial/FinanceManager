...
public partial class BondAccountDetailsPageContent : ComponentBase, IAsyncDisposable
{
    ...
    public async Task UpdateInfo()
    {
        ...
        // Bond entry Value/ValueChange are unit-denominated, so the hero balance and
        // balance-change card are sourced from the currency-valued closing-balance series
        // (the same data the chart plots) rather than from entry values.
        _currentBalance = ChartData.Count > 0 ? ChartData.Last().Value : 0;
    ...
    private async Task LoadBondDetails()
    {
    ...
    private List<BondAccountEntry> GetFilteredEntries()
    {
        ...
        if (!string.IsNullOrWhiteSpace(_searchText))
        {
            ...
            entries = entries.Where(x =>
                BondName(x.BondDetailsId).Contains(needle, StringComparison.OrdinalIgnoreCase)
    ...
    private string BondName(int bondDetailsId) =>
        _bondDetails.FirstOrDefault(x => x.Id == bondDetailsId)?.Name ?? "Bond";
