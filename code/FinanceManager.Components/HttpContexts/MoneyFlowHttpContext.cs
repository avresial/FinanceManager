...
public class MoneyFlowHttpContext(HttpClient httpClient)
{
    ...
    {
        string endpoint = $"{httpClient.BaseAddress}api/MoneyFlow/GetBalance/{userId}/{currency}/{start:O}/{end:O}";
    ...
    {
        string endpoint = $"{httpClient.BaseAddress}api/MoneyFlow/GetIncome/{userId}/{currency}/{start:O}/{end:O}";
        var result = await httpClient.GetFromJsonAsync<List<TimeSeriesModel>>(endpoint);
        return result ?? new List<TimeSeriesModel>();
    }
    ...
    {
        var result = await httpClient.GetFromJsonAsync<decimal?>($"{httpClient.BaseAddress}api/MoneyFlow/GetNetWorth/{userId}/{currency}/{date:O}");
        return result;
    }

    public async Task<Dictionary<DateTime, decimal>> GetNetWorth(int userId, string currency, DateTime start, DateTime end)
    {
        var result = await httpClient.GetFromJsonAsync<Dictionary<DateTime, decimal>>($"{httpClient.BaseAddress}api/MoneyFlow/GetNetWorth/{userId}/{currency}/{start:O}/{end:O}");
    ...
    {
        string endpoint = $"{httpClient.BaseAddress}api/MoneyFlow/GetSpending/{userId}/{currency}/{start:O}/{end:O}";
        var result = await httpClient.GetFromJsonAsync<List<TimeSeriesModel>>(endpoint);
        return result ?? new List<TimeSeriesModel>();
    }
