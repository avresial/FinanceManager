...
public class AccountRepositoryBatchTests
{
    ...
    private static void AddAccount(AppDbContext ctx, int accountId, AccountType type, AccountLabel label = AccountLabel.Other) =>
        ctx.Accounts.Add(new FinancialAccountBaseDto { AccountId = accountId, UserId = UserId, Name = $"Account {accountId}", AccountType = type, AccountLabel = label });

    private static readonly DateTime Start = new(2025, 1, 10, 0, 0, 0, DateTimeKind.Utc);
    private static readonly DateTime End = new(2025, 1, 20, 0, 0, 0, DateTimeKind.Utc);
    ...
    public async Task GetAccounts_Currency_LoadsEachAccountsOwnEntries_OrderedNewestFirst()
    {
        ...
        var accounts = await repo.GetAccounts<CurrencyAccount>(UserId, Start, End).ToListAsync(ct);
    ...
    public async Task GetAccounts_Currency_SetsNextOlderAndNextYoungerBoundaries()
    {
        ...
        ctx.CurrencyEntries.Add(new CurrencyAccountEntry(1, 90, new DateTime(2025, 1, 5, 0, 0, 0, DateTimeKind.Utc), 10, 10));   // older
        ctx.CurrencyEntries.Add(new CurrencyAccountEntry(1, 100, new DateTime(2025, 1, 15, 0, 0, 0, DateTimeKind.Utc), 60, 50)); // in range
        ctx.CurrencyEntries.Add(new CurrencyAccountEntry(1, 110, new DateTime(2025, 1, 25, 0, 0, 0, DateTimeKind.Utc), 80, 20)); // younger
        await ctx.SaveChangesAsync(ct);

        var repo = CreateRepository(ctx);
        var account = (await repo.GetAccounts<CurrencyAccount>(UserId, Start, End).ToListAsync(ct)).Single();
    ...
    public async Task GetAccounts_Currency_ReturnsEmpty_ForUserWithNoAccounts()
    {
        ...
        Assert.Empty(await repo.GetAccounts<CurrencyAccount>(UserId, Start, End).ToListAsync(ct));
    }
    ...
    public async Task GetAccounts_Stock_ResolvesNextOlderPerIsin()
    {
        ...
        var repo = CreateRepository(ctx);
        var account = (await repo.GetAccounts<StockAccount>(UserId, Start, End).ToListAsync(ct)).Single();
    ...
    public async Task GetAccounts_Stock_KeepsInRangeEntries_PerAccount()
    {
        ...
        var repo = CreateRepository(ctx);
        var accounts = await repo.GetAccounts<StockAccount>(UserId, Start, End).ToListAsync(ct);
    ...
    public async Task GetAccounts_Bond_ResolvesNextOlderPerBond()
    {
        ...
        var repo = CreateRepository(ctx);
        var account = (await repo.GetAccounts<BondAccount>(UserId, Start, End).ToListAsync(ct)).Single();
