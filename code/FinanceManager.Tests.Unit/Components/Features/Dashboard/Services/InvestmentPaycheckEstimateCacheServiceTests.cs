...
[Trait("Category", "Unit")]
public class InvestmentPaycheckEstimateCacheServiceTests
{
    private const int _userId = 4;
    private const int _currencyId = 0;
    private const decimal _withdrawalRate = 0.04m;
    private const int _salaryMonths = 3;
    private const string _storageKey = "investment-paycheck-estimate-cache-v1:4:0:2026-07-29:0.0400:3";
    ...
    public async Task SecondVisitOnTheSameDay_ReadsTheEntryTheFirstVisitWrote()
    {
        ...
        var written = new Dictionary<string, InvestmentPaycheckEstimateCacheSnapshot>();
        localStorage
            .Setup(s => s.SetItemAsync(It.IsAny<string>(), It.IsAny<InvestmentPaycheckEstimateCacheSnapshot>(), It.IsAny<CancellationToken>()))
            .Callback<string, InvestmentPaycheckEstimateCacheSnapshot, CancellationToken>((key, snapshot, _) => written[key] = snapshot);
        localStorage
            .Setup(s => s.GetItemAsync<InvestmentPaycheckEstimateCacheSnapshot>(It.IsAny<string>(), It.IsAny<CancellationToken>()))
    ...
    public async Task VisitOnTheNextDay_SupersedesTheStoredEntry()
    {
        ...
        localStorage.Verify(
            s => s.SetItemAsync(
                "investment-paycheck-estimate-cache-v1:4:0:2026-07-30:0.0400:3",
                It.IsAny<InvestmentPaycheckEstimateCacheSnapshot>(),
    ...
    private static InvestmentPaycheckEstimateRefreshContext Context(DateTime endDateTime) => new()
    {
        UserId = _userId,
    ...
    // these tests stay about key shape rather than about estimate maths.
    private static InvestmentPaycheckEstimateCacheService CreateService(Mock<ILocalStorageService> localStorage) =>
        new(localStorage.Object,
            new MemoryCache(new MemoryCacheOptions()),
            new AssetsHttpClient(new HttpClient(new EmptyJsonObjectHandler()) { BaseAddress = new Uri("http://localhost/") }),
            NullLogger<InvestmentPaycheckEstimateCacheService>.Instance);

    private sealed class EmptyJsonObjectHandler : HttpMessageHandler
    {
