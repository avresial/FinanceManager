...
internal class InMemoryBankAccountRepository(AppDbContext context) : IBankAccountRepository<BankAccount>
{
    ...
    public async Task<int> GetAccountsCount() => await _dbContext.Accounts.CountAsync();
    ...
    {
        var toRemove = await _dbContext.Accounts.Where(x => x.AccountId == accountId && x.AccountType == AccountType.Bank).ToListAsync();
    ...
    public Task<bool> Exists(int accountId) => _dbContext.Accounts.AnyAsync(x => x.AccountId == accountId && x.AccountType == AccountType.Bank);

    public async Task<BankAccount?> Get(int accountId)
    {
        var accountToReturn = await _dbContext.Accounts.FirstOrDefaultAsync(x => x.AccountId == accountId && x.AccountType == AccountType.Bank);
    ...
    {
        var bankAccount = await _dbContext.Accounts.FirstOrDefaultAsync(x => x.AccountId == accountId && x.AccountType == AccountType.Bank);
