...
namespace FinanceManager.Tests.Integration.Features.FinancialAccounts.Bond.Controllers;
...
public class BondAccountControllerTests(OptionsProvider optionsProvider) : ControllerTests(optionsProvider), IDisposable
{
    ...
    public async Task GetById_ForOtherUsersAccount_ReturnsForbidden()
    {
        ...
        var response = await Client.GetAsync($"api/BondAccount/{_testAccountId}", TestContext.Current.CancellationToken);
    ...
    public async Task GetRecentEntries_ReturnsMostRecentEntriesWithOlderMetadata()
    {
        ...
        Assert.True(account.NextOlderEntries.ContainsKey(101));
        Assert.Equal(1, account.NextOlderEntries[101].EntryId);
    }
