...
### Security
- The "forgot password" page no longer reveals whether an email is registered: it now shows a reset link for any submitted address (the link only works for a real account), so the page can't be used to discover which emails have accounts. #342
