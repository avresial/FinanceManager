...
- Account history toolbar reorganized into two clean rows: the search field shares the top row with the single filled "Add entry" call-to-action pinned beside it, while the income/expense chips and category filter (an outlined control that highlights when active) sit on the second row next to one quiet overflow "⋮" menu. The overflow now holds import, export, manage, and — on mobile — insights, leaving "Add entry" as the only accent control. #238
