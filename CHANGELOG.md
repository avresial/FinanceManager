...
- The admin AI Providers page now shows whether each provider has an API key configured. #653
- Investment account pages now open as quickly as currency and bond accounts. The trade list repaints its last-rendered trades — with the gain/loss figures that go with them — instantly on re-navigation instead of showing a skeleton for the whole load, and the first uncached view no longer waits on a chain of requests: the account name, the trades and the server-priced valuations are now fetched together, and the benchmark lookup no longer holds up the list. A failed refresh keeps the trades on screen rather than blanking them to "No transactions yet". #646
- The **Subscriptions** page now repaints its last-rendered summary tiles and subscription list instantly on re-navigation, then reconciles against its usual request in the background and only redraws what actually changed. A failed refresh keeps the cached page visible instead of blanking to placeholders, and muting, cancelling or flagging a subscription updates the cached view straight away. #637
- The balance chart on currency, bond, and investment account pages now animates as it draws and carries labelled X and Y axes with gridlines, matching the dashboard cards. The value axis is scaled to the balances on screen rather than starting at zero, so small movements stay visible, and its tick precision adapts to the account — thousands are shown compactly (`28k`) while a small balance keeps the decimals it needs. #634
- Currency, bond, and investment account charts now repaint their last-rendered series and summary values instantly on re-navigation, reconcile through their existing independent requests, and keep cached chart state visible when refresh fails. Each selected range caches its own chart, so switching ranges never shows another range's series. #616
- The Assets page **Asset diversification** card now repaints its last-rendered score instantly on re-navigation, reconciles against a fresh daily request without needless redraws, and keeps the cached score visible when refresh fails. #616
- The dashboard **Financial insights**, **Recurring transactions**, and **Transaction log** cards now repaint their last-rendered state instantly on re-navigation, reconcile against fresh requests without needless redraws, and keep cached content visible when a refresh fails. Each card remains independently scoped to its user and context. #616
...
- Fetching the security token the app needs before signing out or renewing a sign-in no longer counts against the strict sign-in rate limit. That token request shared the same small budget as sign-in itself, so every renewal or sign-out quietly cost two attempts instead of one — halving how many you could make before hitting a "too many requests" error. The token request is now covered only by the app's general request limit. #665
- Renewing a sign-in in the background no longer throws away the cached CSRF token. Because a renewal keeps you signed in as the same person, it used to clear the token needlessly and then fetch a fresh one on the next protected action — an extra request that also counted against the strict sign-in rate limit. The token is now kept across a renewal and only cleared when you actually sign in or out. #666
- Reloading the sign-in page no longer counts against the sign-in rate limit. The app used to ask the server to restore a session on every load, even when you were signed out and there was nothing to restore, so a few reloads on their own could produce a "too many requests" error before you had even entered a password. It now checks whether a session exists first and skips the request when it doesn't. Returning within the sign-in window still logs you straight back in. #667
- Being signed out with a "too many requests" error shortly after returning to an idle tab is now much less likely. When a sign-in expired while several parts of a page were loading, each one asked for a new session separately — a handful of them was enough to exhaust the sign-in rate limit, which ended the session and then blocked the next sign-in attempt too. Those simultaneous requests now share a single renewal. #664
- AI insights no longer go stale for weeks at a time. Generation used to be requested only when you typed your password in again, so a session kept alive in the background could keep showing insights that were months old. Insights older than 7 days are now refreshed in the background the next time you open the dashboard, and repeated visits collapse into a single regeneration. #661
- Investment pricing no longer asks market-data providers for Saturday and Sunday closes. Exchanges are shut then, so those requests always came back empty, burned the free tier's rate limit, and left the window looking "missing" so the same request repeated. Price windows are now trimmed to trading days before the provider is called, a weekend-only window skips the call entirely, and a weekend date is priced from that week's Friday close — so a backfill that restarts over the weekend recovers instead of reporting every instrument as missing prices. #659
- Stock price refreshes no longer call Alpha Vantage's premium adjusted-daily endpoint, so free licenses use the available daily series and can continue to the configured backup providers when needed.
- The benchmark on an investment account's chart now means something. It was drawn with the balance area's fade-to-transparent gradient so no line appeared at all, and it was rebased onto a single opening value — comparing a fixed sum against an account that also grows by contributions, which pinned it to the bottom of the chart. The benchmark now follows the same contributions as the account, moving the same money in and out of Polish inflation (or the investment chosen in **Settings → Preferences**) on each trade date, so the gap between the two lines is the account's real over- or under-performance. #640
- The **Investment rate** and **Investment paycheck** cards now actually reuse their cached data: leaving the Assets page and coming back reads the entry the previous visit stored instead of re-requesting everything and stranding a new browser-storage entry each time. Cached data is scoped to the day it was fetched for, entries a new day supersedes are cleaned up, and the dashboard and assets range caches no longer key on the current clock either. #635
- A failed **Financial insights** request no longer looks like "you have no insights": the card keeps showing the insights you last saw and reports the error, instead of blanking and staying blank on the next visit too. #616
