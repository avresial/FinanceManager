...
### Changed
- "Money by label" dashboard card (formerly "Labels") redesigned from a plain list into a polished, ranked breakdown: a net total in the header, separate Income and Spending groups each with a subtotal, compact rows with a tinted category icon, the signed amount in tabular figures (green for income, red for spending), a proportion bar scaled within its group, and a "% of income/spending" share. Loading now shows skeleton rows and failures show an inline error with a "Try again" retry. #285
