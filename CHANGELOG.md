...
- Account history toolbar restyled for a coherent look: the category filter now matches the income/expense chips as an outlined control (highlighted when active), and the action buttons (insights, import, export, manage) are uniform outlined icon buttons. On mobile the action row is denser — insights and "Add entry" collapse to icon-only buttons that stay on a single row. #238
