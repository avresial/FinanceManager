...
### Security
- Account access controls hardened: changing your own password now requires entering your current password (verified server-side); a non-admin account can no longer delete or change the password of any account other than its own; the admin metrics/user-enumeration endpoints now require the Admin role instead of any signed-in user; and every response now carries baseline security headers (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, and `Permissions-Policy` disabling camera/microphone/geolocation). The API also restricts accepted host headers and only disables HTTPS metadata enforcement in Development. #360
