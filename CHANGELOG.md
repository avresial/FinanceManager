...
- Account history toolbar restyled for a coherent look: the category filter now matches the income/expense chips as an outlined control (highlighted when active), and the action buttons are uniform outlined icon buttons with a single filled "Add entry" call-to-action. On mobile the action row is condensed to three buttons — insights, an overflow "⋮" menu that holds import/export/manage, and an icon-only "Add entry" — that stay on a single row. #238
