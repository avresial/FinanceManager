...
### Changed
- Stock-type accounts now always open in the investment details view, and the legacy stock-account UI has been retired as the first step of the stock → investment cutover. The old per-ticker transaction-history page, its add/update/remove stock-entry forms, and the CSV stock-entry import have been removed; opening any stock-type account renders the investment holdings/transactions view, and the import page for those accounts now explains that transactions are added from the details page. Renaming and deleting a stock-type account is unchanged. #496
