...
- Bond account details page redesigned to match the currency layout: the same hero (avatar, name, currency-valued balance, range toggle, chart), a search/filter toolbar with income/expense chips and category picker, day-grouped transaction cards, and an insight rail (balance change, top/bottom movers) with a slide-in insights drawer on mobile. Bond rows show position and per-entry amounts in units (bond entries are unit-denominated), while the hero and balance-change card report the portfolio value in account currency. #231
