...
### Added
- New users with no accounts now land on a welcoming first-run screen instead of being dropped straight onto a blank "Add account" form: a short intro to what FinanceManager does, one-click "Create" buttons for a Currency, Stock, or Bond account (each opening the form with that type preselected), a carousel previewing the dashboard and account pages, and an "Explore the demo with sample data" shortcut. #281
