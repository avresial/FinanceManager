...
### Changed
- Instrument search now resolves a pasted **ISIN** deterministically: when the search term is a valid ISIN (format plus checksum), the resolver looks the instrument up directly through OpenFIGI's ISIN mapping and uses only the resolved ticker to fetch the Alpha Vantage price handle, instead of fanning out to both providers and reconciling them by fuzzy region matching. Free-text and ticker searches are unchanged. #471
